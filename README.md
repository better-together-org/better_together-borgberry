...
- `ModelRegistry#apply_model_decorations!` — prepends decoration modules onto `BetterTogether::Person`,
  `BetterTogether::Community`, `BetterTogether::PlatformConnection`, and `BetterTogether::Joatu::Agreement` (see
...
This gem's specs reuse the host `community-engine-rails` repo's dummy Rails app and `spec/rails_helper.rb`
(shared factories, database config, etc.) rather than shipping a duplicate dummy app. By default this
expects a sibling checkout at `../community-engine-rails` next to this gem; override with the
`BETTER_TOGETHER_PATH` environment variable if your checkout lives elsewhere:

```bash
bundle install
BETTER_TOGETHER_PATH=/path/to/community-engine-rails bundle exec rspec
```
